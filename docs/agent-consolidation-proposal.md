...
| P4  | Partial | alkyone isolated test infra + **prod-URL/prod-vault guard DONE** (`alkyone/src/alkyone/guard.py`; `make guard`; wired into conftest + `make test-int`); alkyone now linted in per-PR CI (`make -C alkyone lint`); manual `alkyone.yml` dispatch workflow added. `architecture.md` → Repository pattern ✓. TODO: `testing.md`/`architecture.md` VPS rewrite |
| maia DRY | Done | `fffc918` (2026-07-14): `run_agent_main`/`cli_bootstrap` + `video_id_of`/`channel_ids_of` in `maia/utils.py`; `BaseBatchAgent.clear_quota_after_cycle` replaces 4 identical `after_cycle` overrides; tests re-patched to `maia.base.clear_quota_exhausted`. 110 maia unit tests green, ruff clean. High-risk refactors (flow base class, `commit_artifacts` fold-in, `TemporaryDirectory` swap) deferred. |
| painter/singer starvation | Diagnosed + store regression fixed | **Not a scheduling bug** (orchestration `concurrency_limit=9` is correct). Two findings: (1) the *streamer* is ~80% failing YouTube `HTTP 403 Forbidden` on the flagged VPS egress IP (~40 fetched/day vs ~3600/day theoretical) — input starvation; (2) **store regression fixed 2026-07-14**: `commit_artifacts` was called with the `get_vault` factory (not instance) → `AttributeError` → every streamer/singer/muralist batch store failed to persist, so fetched bytes never became `raw_uri`. Fix in `maia/storage.py` + callers (`get_vault()`), regression test in `maia/tests/test_storage.py`. Scribe bypasses both via free captions (14,133 transcripts). See `micro-prefect-orchestration.md` §7. |

...
def test_preprocess_tags(sample_df):
    """Test that tags string is correctly split into lists."""
    result = preprocess_tags.fn(sample_df)
...
def test_load_data(MockDataLoader):
...
def test_generate_rules(sample_dataset):
    ...
    rules = generate_rules.fn(sample_dataset)
...
@patch("joblib.dump")
def test_validate_and_upload_success(mock_dump, MockUploader):
    ...
    status = validate_and_upload.fn(rules, "report.html")
...
@patch("training.pipelines.tags_pipeline.ModelUploader")
def test_validate_and_upload_failure(MockUploader):

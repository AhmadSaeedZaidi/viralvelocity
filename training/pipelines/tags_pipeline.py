...
def train_model(dataset):
    ...
    if frequent_itemsets.empty:
        logger.warning("No frequent itemsets found! Relaxing support to 0.005...")
        frequent_itemsets = apriori(df_trans, min_support=0.005, use_colnames=True)
        if frequent_itemsets.empty:
            raise ValueError("No frequent itemsets found even with low support.")
...
def run_evaluation_checks(rules, rule_metrics):
    ...
    if rule_metrics["avg_lift"] < 1.05:
        logger.warning(
            f"Average lift is too low ({rule_metrics['avg_lift']}). " "Rules are weak."
        )
        return False
...
def validate_and_upload(rules, rule_metrics, is_valid, reports):
    ...
    try:
        ...
        bounds = {"rule_count": (5, 10000), "avg_lift": (1.05, 100.0)}
...
def tags_training_flow():
    ...
    try:
        ...
        if not passed:
            logger.warning("Data Integrity Checks Failed. Continuing pipeline...")

...
async def evaluate_predictions(payload: EvaluationRequest):
    ...
    if payload.task_type == "regression":
        ...
        # Round predictions for classification if they are probabilities
        y_pred_class = np.round(y_pred)
        
        metrics = {
            "accuracy": float(accuracy_score(y_true, y_pred_class)),
            "f1": float(f1_score(y_true, y_pred_class, average=average, zero_division=0)),
            "precision": float(precision_score(y_true, y_pred_class, average=average, zero_division=0)),
            "recall": float(recall_score(y_true, y_pred_class, average=average, zero_division=0))
        }

...
def test_compute_sizes_small_corpus():
    """Small corpus: balanced split, tracker protected, hunter keeps the bulk."""
    sizes = compute_sizes(total_keys=24, video_count=1_342, archeology_size=3)
    assert sizes.archeology == 3
    assert sizes.tracking == 8
...
def test_compute_sizes_large_corpus_shifts_to_tracking():
    ...
    assert sizes.archeology == 3
    assert sizes.tracking == 14
    assert 24 - sizes.tracking - sizes.archeology == 7
...
def test_archeology_reserve_clamped():
    """Archeology reserve is floored at 2 and cannot consume the whole pool."""
...
def test_refresh_writes_and_loads(tmp_path):
    ...
    assert sizes == PoolSizes(tracking=14, archeology=3)

    loaded = load_override(path)
    assert loaded == PoolSizes(tracking=14, archeology=3)
...
def test_refresh_respects_freshness(tmp_path):
    ...
    assert load_override(path) == PoolSizes(tracking=8, archeology=3)

...
class MaiaDAO(DatabaseAdapter, GhostTrackingMixin):
    ...
    async def ingest_video_metadata(
        ...
        if channel_id and channel_title:
             await self._execute(channel_query, (channel_id, channel_title, datetime.now(timezone.utc)))
        ...
        await self._execute(video_query, (
            vid_id, channel_id, title, published_at, 
            tags, category_id, default_language, 

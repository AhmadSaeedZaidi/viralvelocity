...
class DatabaseAdapter:
    ...
    async def _fetch_all(self, query: str, params: Optional[Tuple] = None) -> List[Dict[str, Any]]:
        async with self._cursor() as cur:
            await cur.execute(query, params or ())
            columns = [desc[0] for desc in cur.description] if cur.description else []
    ...
    async def _fetch_many(self, query: str, params: Optional[Tuple], limit: int) -> List[Dict[str, Any]]:
        async with self._cursor() as cur:
            await cur.execute(query, params or ())
            columns = [desc[0] for desc in cur.description] if cur.description else []

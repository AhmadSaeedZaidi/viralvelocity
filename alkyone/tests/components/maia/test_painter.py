...
async def test_painter_real_full_cycle_blender_tutorial(dao):
    ...
    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
        try:
            ...
            print(f"[Test] Video '{info.get('title')}' - Chapters: {has_chapters}, Heatmap: {has_heatmap}")
    ...
    await dao._execute("UPDATE videos SET has_visuals = FALSE, status = 'PENDING' WHERE id = %s", (video_id,))
